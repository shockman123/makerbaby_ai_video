...
function updateQwenModel(nodeId, model) {
    ...
    node.videoRatio = null;
    renderNode(node);
    scheduleSave();
}

// 视频生成模型（不含 videoedit）—— 普通视频生成节点使用
let qwenVideoModels = [
    // i2v 图生视频
...
function getModelTypeNote(m) {
    if (m.type === 'i2v') return '（图生视频·仅首帧1张）';
    if (m.type === 't2v') return '（文生视频·无需图片）';
    if (m.type === 'r2v') return '（参考图生视频·支持多张）';
    if (m.type === 'videoedit') return '（视频生视频）';
    return '';
}

function getQwenModelOptions(selectedId, isVideoEdit, t2vOnly) {
    const selectedIsEdit = qwenVideoEditModels.some(m => m.id === selectedId);
    const useEditList = isVideoEdit || selectedIsEdit;
    let models = useEditList ? qwenVideoEditModels : qwenVideoModels;
    if (t2vOnly) {
        models = models.filter(m => m.type === 't2v');
    }
    return models.map(m => {
        const note = getModelTypeNote(m);
...
const IMAGE_SIZE_MAP = {
    'agnes': { '1:1': '1024x1024', '16:9': '1024x576', '9:16': '576x1024' },
    'wan2.7-image-pro': { '1:1': '1024*1024', '16:9': '1280*720', '9:16': '720*1280' },
    'wan2.7-image': { '1:1': '1024*1024', '16:9': '1280*720', '9:16': '720*1280' },
    'qwen-image-2.0-pro-2026-04-22': { '1:1': '2048*2048', '16:9': '2688*1536', '9:16': '1536*2688' },
};

// 视频参数（简化：比例 + 时长）
const VIDEO_PARAMS = {
    agnes: { ratios: ['16:9', '9:16', '1:1'], durations: [5, 8, 10] },
    'wan2.7': { ratios: ['16:9', '9:16', '1:1'], durations: [5, 8] },
    happyhorse: { ratios: ['16:9', '9:16', '1:1'], durations: [5, 8] },
    videoedit: { durations: [0, 5, 8] },
};

function getVideoParamKey(model, qwenModel) {
    // model: 'agnes' 或 'qwen'（视频平台）
    // qwenModel: 千问模型ID（仅千问平台有）
    if (model === 'qwen') {
        if (!qwenModel) return 'wan2.7';
        if (qwenModel.includes('videoedit') || qwenModel.includes('video-edit')) return 'videoedit';
        if (qwenModel.startsWith('happyhorse')) return 'happyhorse';
        if (qwenModel.startsWith('wan2.7')) return 'wan2.7';
        return 'wan2.7';
    }
    // 默认（agnes 或未设置）使用 Agnes 参数
    return 'agnes';
}

function renderParamButtons(label, options, selectedValue, onChangeFn, nodeId) {
    const buttons = options.map(opt => {
        const display = String(opt);
        const isActive = String(selectedValue) === display;
        return `<button class="param-btn ${isActive ? 'active' : ''}" onclick="event.stopPropagation();${onChangeFn}('${nodeId}', '${opt}')">${display}</button>`;
    }).join('');
    return `<div class="param-row"><span class="param-label">${label}</span>${buttons}</div>`;
}

function updateVideoRatio(nodeId, val) {
    const node = state.nodes.find(n => n.id === nodeId);
    console.log('[updateVideoRatio] nodeId=', nodeId, 'val=', val, 'found=', node ? 'yes' : 'no');
    if (node) { node.videoRatio = val; scheduleSave(); renderNode(node); }
}
function updateVideoResolution(nodeId, val) {
    const node = state.nodes.find(n => n.id === nodeId);
    if (node) { node.videoResolution = val; scheduleSave(); renderNode(node); }
}
function updateVideoDuration(nodeId, val) {
    const node = state.nodes.find(n => n.id === nodeId);
    if (node) { node.videoDuration = parseInt(val); scheduleSave(); renderNode(node); }
}
function updateImageRatio(nodeId, val) {
    const node = state.nodes.find(n => n.id === nodeId);
    if (node) { node.imageRatio = val; scheduleSave(); renderNode(node); }
}
...
function renderAssetPanel() {
    ...
    // 渲染底部的 Task ID 下载区域
    renderTaskRecoverPanel();
}

function renderTaskRecoverPanel() {
    const panel = document.getElementById('task-recover-panel');
    if (!panel) return;
    ...
    panel.innerHTML = `
        <div style="margin-top:12px; padding:10px; background:#f5f5f5; border-radius:6px;">
            ...
            <input type="text" id="recover-task-input" placeholder="输入 task_id (如 task_xxx 或纯数字)"
                style="width:100%; padding:6px; border:1px solid #ddd; border-radius:4px; font-size:11px; box-sizing:border-box;">
            <div style="display:flex; gap:4px; margin-top:6px;">
                <select id="recover-platform-select"
                    style="flex:1; padding:5px; border:1px solid #ddd; border-radius:4px; font-size:11px;">
                    <option value="agnes">Agnes</option>
                    <option value="qwen">千问</option>
                </select>
                <button onclick="recoverTaskVideo()"
                    style="flex:1; padding:5px 10px; background:#4CAF50; color:white; border:none; border-radius:4px; cursor:pointer; font-size:11px;">
                    下载视频
                </button>
            </div>
            <div id="recover-status" style="margin-top:6px; font-size:11px; color:#666;"></div>
        </div>
    `;
}

async function recoverTaskVideo() {
    const taskIdInput = document.getElementById('recover-task-input');
    const platformSelect = document.getElementById('recover-platform-select');
    const statusDiv = document.getElementById('recover-status');

    let taskId = taskIdInput.value.trim();
    if (!taskId) {
        statusDiv.innerHTML = '<span style="color:#f44336;">❌ 请输入 task_id</span>';
        return;
    }

    // 自动补充 task_ 前缀（如果是 Agnes 纯数字 ID）
    const platform = platformSelect.value;
    if (platform === 'agnes' && !taskId.startsWith('task_')) {
        taskId = 'task_' + taskId;
    }

    statusDiv.innerHTML = '<span style="color:#2196F3;">⏳ 正在查询并下载...</span>';

    try {
        const res = await fetch(`/api/task/${taskId}/recover`, {
            method: 'POST',
        ...
        if (data.success) {
            statusDiv.innerHTML = `<span style="color:#4CAF50;">✅ ${data.message || '下载成功'}</span>`;
            taskIdInput.value = '';
            // 刷新资产列表
            await loadAssets();
        } else {
            const errMsg = data.error || '下载失败';
            const expired = data.expired ? ' (任务已过期)' : '';
            statusDiv.innerHTML = `<span style="color:#f44336;">❌ ${errMsg}${expired}</span>`;
        }
    } catch (e) {
        statusDiv.innerHTML = `<span style="color:#f44336;">❌ 网络错误: ${e.message}</span>`;
    }
}
...
function createNextNode(sourceNodeId, type) {
    ...
    const node = {
        ...
        model: type === 'image' ? 'qwen' : 'agnes',  // 图片默认千问，视频默认Agnes
        x: sourceNode.x + 300,
        y: sourceNode.y,
        prompt: '',
        size: '1024x768',
...
// 从视频资产创建视频生视频节点（使用千问视频编辑模型）
function createVideoGenFromAsset(assetId) {
    const sourceNode = state.nodes.find(n => n.id === assetId);
    if (!sourceNode) return;
    if (sourceNode.assetType !== 'video') {
        alert('只有视频资产才能使用此功能');
        return;
    }
    ...
    const genNode = {
        ...
        prompt: '',
        model: 'qwen',  // 默认使用千问
        qwenModel: 'wan2.7-videoedit',  // 默认用视频编辑模型
        isVideoEdit: true,  // 标记为视频生视频节点，只显示视频编辑模型
        outputAsset: null,
        status: 'idle'
    };

    state.nodes.push(genNode);
    ...
    if (node.type === 'asset') {
        ...
        bodyHtml = `
            ...
            <div class="node-action-btns">
                ...
                ${!isImage ? `<button class="action-btn action-btn-orange" onclick="event.stopPropagation();createVideoGenFromAsset('${node.id}')">🎬 视频生视频</button>` : ''}
            </div>
        `;
    } else if (node.type === 'image-gen') {
        ...
        bodyHtml = `
            ...
            ` : ''}
            ${(() => {
                let imgModel = node.platform === 'qwen' ? (node.qwenImageModel || 'wan2.7-image-pro') : 'agnes';
                let sizeMap = IMAGE_SIZE_MAP[imgModel];
                if (!sizeMap) return '';
                let currentRatio = node.imageRatio || '1:1';
                let ratioBtns = IMAGE_RATIOS.map(r => `<button class="param-btn ${currentRatio === r ? 'active' : ''}" onclick="event.stopPropagation();updateImageRatio('${node.id}', '${r}')">${r}</button>`).join('');
                return `<div class="param-row"><span class="param-label">比例</span>${ratioBtns}</div>`;
            })()}
        ...
        const qwenModelOptions = getQwenModelOptions(node.qwenModel, node.isVideoEdit === true, node.t2vOnly === true);
        ...
        bodyHtml = `
            ...
            <div class="prompt-row">
                <textarea class="node-prompt-input" placeholder="${node.t2vOnly ? '输入视频描述... 如：一只猫在沙滩上奔跑' : '输入视频描述... 可用 [帧1] [帧2] 标记关键帧'}" onclick="event.stopPropagation()" oninput="updateNodePrompt('${node.id}', this.value)">${node.prompt || ''}</textarea>
            </div>
            <div class="model-select-row">
                <label>平台:</label>
            ...
            ` : ''}
            ${(() => {
                let paramKey = getVideoParamKey(node.model, node.qwenModel);
                let vParams = VIDEO_PARAMS[paramKey];
                if (!vParams) return '';
                let html = '';
                let currentRatio = node.videoRatio || vParams.ratios?.[0] || '16:9';
                let currentDur = node.videoDuration || vParams.durations?.[0] || 5;
                if (vParams.ratios) {
                    html += renderParamButtons('比例', vParams.ratios, currentRatio, 'updateVideoRatio', node.id);
                }
                if (vParams.durations) {
                    let durButtons = vParams.durations.map(d => {
                        let label = d === 0 ? '原长' : d + 's';
                        let isActive = currentDur === d;
                        return `<button class="param-btn ${isActive ? 'active' : ''}" onclick="event.stopPropagation();updateVideoDuration('${node.id}', ${d})">${label}</button>`;
                    }).join('');
                    html += `<div class="param-row"><span class="param-label">时长</span>${durButtons}</div>`;
                }
                return html;
            })()}
            <button class="generate-btn" data-action="generate-video" data-node-id="${node.id}" ${node.status === 'generating' ? 'disabled' : ''}>
                ${node.status === 'generating' ? '查询中...' : (node.manualQuery && node.taskId ? '🔍 查询进度' : '生成视频')}
            </button>
            ${node.manualQuery && node.taskId ? `<button class="action-btn action-btn-gray" onclick="event.stopPropagation();regenerateVideo('${node.id}')">🔄 重新生成</button>` : ''}
            ${node.status === 'generating' ? `<button class="cancel-btn" onclick="event.stopPropagation();cancelVideoGeneration('${node.id}')">取消生成</button>` : ''}
            <div class="progress-bar ${node.status === 'generating' ? 'active' : ''}">
                <div class="progress-fill" style="width: ${node.progress || 0}%"></div>
            </div>
            ${(node.status === 'generating' || (node.manualQuery && node.statusText)) ? `<div class="status-text">${node.statusText || '生成中...'}</div>` : ''}
        `;
    } else if (node.type === 'video-edit') {
        const inputConns = state.connections.filter(c => c.to === node.id);
        const inputVideos = [];
    ...
    try {
        ...
        state.nodes.forEach(node => {
            if (node.type === 'video-gen' && node.taskId && !node.outputAsset) {
                ...
                node.status = 'idle';
                node.statusText = '点击查询按钮检查任务状态';
                renderNode(node);
                // 先验证任务是否有效（一次性检查，不自动轮询）
                verifyAndPollVideo(node, true);
            }
...
// 单次查询（不自动轮询）
async function manualQueryVideo(nodeId) {
    const node = state.nodes.find(n => n.id === nodeId);
    if (!node || !node.taskId) return;
    ...
    try {
        ...
        if (data.success) {
            ...
            if (data.status === 'completed') {
                ...
                node.progress = 100;
                node.statusText = '完成';
                node.manualQuery = false;

                if (data.video_url) {
                    let localUrl, assetId, assetName;
                    if (node.model === 'qwen') {
                        localUrl = `/api/assets/qwen_${node.taskId}`;
                        assetId = 'qwen_' + node.taskId;
                        assetName = `video_qwen_${node.taskId}.mp4`;
                    } else {
                        const videoId = data.video_id || extractVideoIdFromUrl(data.video_url);
                        localUrl = `/api/assets/${videoId}`;
                        assetId = videoId;
                        assetName = videoId.startsWith('video_') ? `${videoId}.mp4` : `video_${videoId}.mp4`;
                    }
                    node.outputAsset = {
                        id: assetId,
                        type: 'video',
                ...
                node.status = 'idle';
                node.statusText = '任务失败: ' + (data.error || '未知');
                renderNode(node);
                return;
            } else if (data.status === 'in_progress' || data.status === 'RUNNING') {
                node.statusText = `生成中... ${node.progress}% (点击查询刷新)`;
            } else if (data.status === 'queued' || data.status === 'pending' || data.status === 'PENDING') {
                if (data.submit_time) {
                    node.statusText = `排队中... 提交: ${data.submit_time.split(' ')[1] || data.submit_time} (点击查询刷新)`;
                } else {
                    node.statusText = '排队中... (点击查询刷新)';
                }
            }
            // 关键：手动查询模式下，无论进行中/排队中，都恢复 idle 状态，让按钮可再次点击
            node.status = 'idle';
            ...
            node.status = 'idle';
            node.statusText = '查询失败: ' + (data.error || '未知错误');
        }
        renderNode(node);
    } catch (e) {
        node.status = 'idle';
        node.statusText = '查询出错: ' + e.message;
        renderNode(node);
    }
}
...
async function generateImage(nodeId) {
    ...
    try {
        // 根据比例映射到实际尺寸
        const imgModel = platform === 'qwen' ? (node.qwenImageModel || 'wan2.7-image-pro') : 'agnes';
        const sizeMap = IMAGE_SIZE_MAP[imgModel] || {};
        const imgSize = sizeMap[node.imageRatio || '1:1'] || '1024x1024';
...
function updateVideoModel(nodeId, model) {
    ...
    if (!node) return;
    node.model = model;
    if (model === 'qwen' && node.t2vOnly && !node.qwenModel) {
        node.qwenModel = 'wan2.7-t2v';
    }
    renderNode(node);
    scheduleSave();
}
...
async function generateVideo(nodeId) {
    ...
    // 手动查询模式：点击"查询进度"按钮 → 单次查询，不重新生成
    if (node.manualQuery && node.taskId) {
        await manualQueryVideo(nodeId);
        return;
    }
    ...
    for (const conn of inputConns) {
        ...
        if (sourceNode) {
            ...
            } else if (sourceNode.type === 'asset' && sourceNode.assetType === 'video') {
                videoAssetIds.push(sourceNode.assetId);
            } else if (sourceNode.outputAsset && sourceNode.outputAsset.type === 'video') {
                videoAssetIds.push(sourceNode.outputAsset.id);
            }
        }
    }
    ...
    try {
        ...
        // 传递参数
        console.log('[Generate Video] node.videoRatio=', node.videoRatio, 'node.videoDuration=', node.videoDuration, 'model=', node.model);
        // Agnes 或 model 未设置 → 走 Agnes 分支
        if (!node.model || node.model === 'agnes') {
            bodyData.agnes_resolution = node.videoRatio || '16:9';
            bodyData.agnes_duration = node.videoDuration || 5;
        } else {
            bodyData.qwen_resolution = '720P';
            bodyData.qwen_ratio = node.videoRatio || '16:9';
            bodyData.qwen_duration = node.videoDuration || 5;
        }
        if (node.model === 'qwen') {
...
// 重新生成：清除 manualQuery 状态，触发正常生成流程
function regenerateVideo(nodeId) {
    const node = state.nodes.find(n => n.id === nodeId);
    if (!node) return;
    node.manualQuery = false;
    delete node.taskId;
    ...
    node.progress = 0;
    renderNode(node);
    generateVideo(nodeId);
}

async function verifyAndPollVideo(node, manualMode = false) {
    // 先做一次性健康检查
    try {
        const headers = {};
        if (node.model === 'qwen') {
...
async function pollVideoStatus(nodeId) {
    ...
    const poll = async () => {
        ...
        if (pollCount > MAX_POLLS) {
            // 超时：切换到手动查询模式，让用户可以点击按钮继续查询
            node.status = 'idle';
            node.manualQuery = true;
            node.statusText = '查询超时，点击"查询进度"继续';
            renderNode(node);
            saveWorkflow();
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.status === 'completed') {
                    ...
                    // 任务失败：显示错误信息，保留 taskId 供用户查看，切换到手动查询模式
                    node.status = 'idle';
                    node.manualQuery = true;
                    node.statusText = '❌ 失败: ' + (data.error || '未知错误').substring(0, 100);
                    renderNode(node);
                    saveWorkflow();
                    console.log('[轮询] 视频生成失败:', data.error);
                    return;
                } else if (data.status === 'in_progress') {
...
async function deleteNode(nodeId) {
    ...
    // 如果有 taskId，尝试取消任务（无论 status 是什么）
    if (node && node.taskId && node.model === 'qwen') {
        try {
...
function onDragNode(e) {
    ...
    // 用 requestAnimationFrame 节流连线更新，避免每次 mousemove 都重绘 SVG
    if (!state._dragRafScheduled) {
        state._dragRafScheduled = true;
        requestAnimationFrame(() => {
            state._dragRafScheduled = false;
            renderConnections();
        });
    }
}
...
function renderConnections() {
    // 复用已有 path 元素，避免每次都删除重建（提升拖拽性能）
    const existing = connectionsSvg.querySelectorAll('.connection-line:not(.temp)');
    const conns = state.connections;

    // 多余的 path 删除
    for (let i = existing.length - 1; i >= conns.length; i--) {
        existing[i].remove();
    }
    ...
    conns.forEach((conn, idx) => {
        ...
        const d = createBezierPath(fromPos.x, fromPos.y, toPos.x, toPos.y);

        let path = existing[idx];
        if (!path) {
    ...
    if (node.taskId) {
        if (node.model === 'qwen') {
            try {
                ...
                const data = await res.json();
                console.log('取消千问任务:', data.success ? '成功' : '失败', data.message || data.error);
            } catch (e) {
                console.log('取消千问任务异常:', e.message);
            }
        } else {
            // Agnes 取消
            try {
                const res = await fetch(`/api/agnes/cancel/${node.taskId}`, {
                    method: 'POST',
                    headers: {
                        'X-API-Key': getApiKey(),
                        'Content-Type': 'application/json'
                ...
                const data = await res.json();
                console.log('取消Agnes任务:', data.success ? '成功' : '失败', data.message || data.error);
            } catch (e) {
                console.log('取消Agnes任务异常:', e.message);
            }
        }
    }
